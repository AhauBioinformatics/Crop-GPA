...
git_rev = subprocess.Popen("git rev-parse --short HEAD", shell=True, stdout=subprocess.PIPE,
                           universal_newlines=True).communicate()[0]
git_branch = subprocess.Popen("git symbolic-ref --short -q HEAD", shell=True, stdout=subprocess.PIPE,
                              universal_newlines=True).communicate()[0]
...
parser.add_argument("--log_device_placement", default=False, action="store_true",
                    help="Whether to log device placement")
parser.add_argument("--data_prefix", required=False, default="gene_trait_data/task_Tg__testlabel0_5knn_edge_fold", type=str, help="prefix identifying training data")
...
parser.add_argument("--train_config", default="parameters_epoch_1.yml", required=False, type=str, help="path to the configuration of training (*.yml)")
...
EVAL_VAL_EVERY_EP = 1  # get accuracy on the eval validation set every this epochs

# auto choosing available NVIDIA GPU
gpu_selected = args_global.gpu
if gpu_selected == '-1234':
    # auto detect gpu by filtering on the nvidia-smi command output   通过过滤nvidia-smi命令输出来自动检测gpu
    gpu_stat = \
