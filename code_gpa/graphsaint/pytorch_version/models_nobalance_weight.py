...
class GraphSAINT(nn.Module):
    def __init__(self, num_classes, arch_gcn, train_params, feat_full, label_full, cpu_eval=False):
        ...
        self.positive_weight = torch.Tensor([1, train_params['positive_weight']]).to(device)
        #self.positive_weight = torch.Tensor([1, train_params['positive_weight']]).cuda() ###
        ...
        if self.use_cuda:
            ...
            self.label_full = self.label_full.to(device)
    ...
    def _loss(self, preds, labels, norm_loss, positive_weight): # , positive_weight = torch.Tensor([1, 10]).cuda()
        """
    ...
    def train_step(self, node_subgraph, adj_subgraph, norm_loss_subgraph):
        ...
        loss = self._loss(preds, labels_converted, norm_loss_subgraph, self.positive_weight) # labels.squeeze()?
        loss.backward()
    ...
    def eval_step(self, node_subgraph, adj_subgraph, norm_loss_subgraph):
        ...
        with torch.no_grad():
            preds,labels,labels_converted = self(node_subgraph, adj_subgraph)
            loss = self._loss(preds,labels_converted,norm_loss_subgraph, self.positive_weight)
        return loss, self.predict(preds), labels
